...
type WorksStats = { total: number; newThisWeek: number; totalReads: number; totalLikes: number }
export type WorksViewProps = { initialPosts?: Post[]; initialStats?: WorksStats }

export function WorksView({ initialPosts, initialStats }: WorksViewProps) {
  const { user } = useAuth()
  ...
  const loadBookmarkedPosts = async () => {
    try {
      ...
      const data = (await res.json()) as { postIds: string[] }
