...
type AnalyticsViewProps = { initialAnalytics?: UserAnalytics | null; defaultDays?: string }

function AnalyticsContent({ initialAnalytics, defaultDays = '30' }: AnalyticsViewProps) {
  const [analytics, setAnalytics] = useState<UserAnalytics | null>(initialAnalytics ?? null)
  const [loading, setLoading] = useState(!initialAnalytics)
  const [timeRange, setTimeRange] = useState(defaultDays)

  useEffect(() => {
    // Fetch if no initial data or timeRange changed from default
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/analytics/user?days=${timeRange}`, { cache: 'no-store' })
      if (response.ok) {
