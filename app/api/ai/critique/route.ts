...
async function handleCritique(request: NextRequest) {
  let user: any = null
  try {
    ...
    logger.error('AI Critique request failed', error, {
      userId: user?.id,
...
export const POST = createRateLimitedHandler('ai', handleCritique)
